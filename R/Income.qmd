...
df <- clean_data |>
  mutate(
    Verylow = case_when(income_class == "Very poor" ~ 1, .default = 0),
    Low     = case_when(income_class == "Poor" ~ 1, .default = 0),
    Middle  = case_when(income_class == "Middle class" ~ 1, .default = 0),
    Rich    = case_when(income_class == "Wealthy" ~ 1, .default = 0))
...
df_income <- clean_data |>
  mutate(
    verypoor = case_when(income_class == "Income very low" ~ 1, .default = 0),
    poor     = case_when(income_class == "Income low" ~ 1, .default = 0),
...
```{r}
summary(df_income_stat_models$model[[1]])  #Very poor
summary(df_income_stat_models$model[[2]])  #Poor
summary(df_income_stat_models$model[[3]])  #Middle class
summary(df_income_stat_models$model[[4]])  #Wealthy

```

## Plots

Making boxplot/ histograms

```{r}
boxplots <- med_income_verylow |>
    geom_boxplot(aes(x = death,
               fill = sex,), 
           position = "dodge" +
  labs(title="Distribution of death based on income")
