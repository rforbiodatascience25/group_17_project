...
race_plot2 <- ggplot(df_race_stat,
  ...
  geom_errorbar(aes(ymin=CI_dra_lower,
                    ymax=CI_dra_upper))+
  labs(title = "Death rates of pateints of different races in the study",
       y = 'Death rate',
       x='Race (sample size)',
       subtitle = "Errorbars show 2 standard deviations") +
  scale_x_discrete(labels = function(x) glue("{x} ({df_race_stat$count})"))
