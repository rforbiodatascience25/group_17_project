...
```
QQ plot to compare the residuals to the normal distribution.
The points fall into to almost horizontal lines, instead of following the diagnoal line. 
This confirms that the residuals are not at all normally distributed (which is not very suprising 
since they are binary), and this violates the normality assumption of the linear model.
...
log_aug <- augment(age_log_model, type.predict = "response")

# Plot logistic regression fitted values
ggplot(log_aug, aes(x = age, y = .fitted)) +
  geom_point(alpha = 0.4) +
